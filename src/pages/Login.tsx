      ...
      <div className="max-w-md w-full mx-auto space-y-8 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-2xl backdrop-blur-sm bg-opacity-90 dark:bg-opacity-80 transition-all duration-500">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            {t('or')}{' '}
            <Link to="/register" className="font-medium text-red-primary hover:text-red-dark transition-colors duration-300">
              {t('create_account')}
            </Link>
          </p>
        </div>
        ...
        <div className="mt-8 space-y-6">
          ...
          <GoogleAuth buttonText={t('google')} setIsLoading={setIsLoading} />
        </div>
      </div>
    </div>
  );
